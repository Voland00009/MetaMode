...
| Error | Cause | Action |
|-------|-------|--------|
| Auth/cookie error | Session expired | Re-run login flow (cookies expire every 1-7 days) |
...
- **Cookie expiration is unavoidable** — Google invalidates browser cookies periodically (1-7 days). You will need to re-authenticate.
