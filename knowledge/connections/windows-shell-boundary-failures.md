...
A fifth pattern — **filesystem path resolution mismatch** ([[concepts/python-path-tmp-windows-mismatch]]): bash's `/tmp` maps through MSYS2's virtual filesystem to a platform-specific temp directory, while Python's `Path("/tmp")` resolves to `C:\tmp` (drive root). Inter-process communication via signal files in `/tmp` silently breaks — files created by bash are invisible to Python.

All five share a root cause: Windows was designed around a single-shell model (cmd.exe), and layering bash on top creates boundary mismatches at every interface point — PATH resolution, argument passing, output streams, character escaping, and filesystem paths.
...
4. **Use `Path.home()` for cross-process temp files** — `/tmp` resolves differently in bash and Python. `Path.home()` is one of the few paths both resolve identically.
5. **Test automation on Windows specifically** — these bugs don't exist on macOS/Linux. Scripts that work perfectly on Unix may fail silently on Windows at any of these five boundary points.
...
- [[concepts/python-path-tmp-windows-mismatch]]
